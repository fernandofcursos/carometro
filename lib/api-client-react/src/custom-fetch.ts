...
/**
 * Set a default `credentials` value applied to every fetch request.
 * Use `"include"` for web apps that rely on httpOnly session cookies.
 * Pass `null` to clear the override and revert to browser defaults.
 */
...
/**
 * Register a callback invoked whenever any API call receives a 401 response.
 * Use this to handle session expiry (e.g. redirect to login).
 * Pass `null` to clear the callback.
 */
export function setAuthErrorCallback(cb: (() => void) | null): void {
...
export async function customFetch<T = unknown>(
  ...
  if (!response.ok) {
    if (response.status === 401 && _authErrorCallback) {
      _authErrorCallback();
    }
